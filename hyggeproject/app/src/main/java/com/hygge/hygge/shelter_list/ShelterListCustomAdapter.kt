...
class ShelterListCustomAdapter(val context: Context, val shelterList: ArrayList<Shelter>) :  BaseAdapter() {
    ...
    override fun getView(p0: Int, p1: View?, p2: ViewGroup?): View {   //View와 데이터를 연결
        ...
        val photoId = context.resources.getIdentifier(shelter.photo, "drawable", context.packageName)

        shImage.setImageResource(photoId)
        shTitle.text = shelter.title
        shDescription.text = shelter.description
